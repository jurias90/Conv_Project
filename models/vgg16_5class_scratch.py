...
CM_PATH  = f"{BASE}/results/confusion_matrix_vgg16_5class_scratch.png"
LOG_PATH = f"{BASE}/results/vgg16_5class_scratch_log.json"
...
def main():
    ...
    # ─────────────────────────────────────────
    model = vgg16(weights=None)  # ← random initialization

    # All layers trainable — nothing to freeze
    # (no pretrained knowledge to protect)
    ...
    # 20 epochs to give scratch model fair chance to converge
    # ─────────────────────────────────────────
    criterion = nn.CrossEntropyLoss()

    # Single learning rate — no separate rates needed
    # since there are no pretrained layers to protect
    ...
    EPOCHS = 20  # more epochs needed when starting from scratch
    ...
    # Save training log for comparison plots
    ...
    ax.set(xticks=np.arange(cm.shape[1]),
           ...
           xticklabels=classes,
           yticklabels=classes,
           title=f'VGG16 5-class FROM SCRATCH — {final_acc:.2%}\n'
